...
{
    if (size == capacity)
        data = (char*)realloc(data, capacity + SLOT);//не эффективно уменьшить колво реалоков
...
{
    if (size * 3 == capacity)
        data = (char*)realloc(data, capacity / 3);
    size--;
}
...
void StackDestroy (Stack_t *stack)
{
    ...
    stack->data = NULL;
    free(stack->data);// обнулить указатель, cделать статусную переменную
}
...
void StackCreate(Stack_t *stack)
{
    ...
    stack->error = 0;
    stack->size = 0;
    stack->data = (char*)calloc(stack->capacity, sizeof(char));//Тут я создаю calloc нулевого размера , может быть лажа в отдельную функцию
}
